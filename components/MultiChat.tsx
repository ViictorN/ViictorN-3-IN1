...
const MultiChat: React.FC<MultiChatProps> = ({ activeStreamers, isOpen, onClose, width, onResize }) => {
      ...
      {isOpen && (
        <>
            ...
            <div 
                ...
                className="absolute left-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-white/20 transition-colors z-50 group flex items-center justify-center"
            >
                <div className="h-8 w-[2px] bg-white/20 group-hover:bg-white/50 rounded-full" />
            </div>

            {/* HEADER TABS */}
            <div className="flex flex-none items-end h-16 bg-black border-b border-white/5 px-2 gap-1 overflow-x-auto no-scrollbar pl-4">
                
                {/* MIX TAB */}
                <button
                onClick={() => setSelectedStreamerId('all')}
                className={`
                    relative group flex flex-col items-center justify-center px-2 h-12 min-w-[60px] rounded-t-lg transition-all duration-300
                ...
                <div className="w-[1px] h-5 mb-3.5 bg-white/10 mx-1 self-end flex-shrink-0" />
                ...
                return (
                    ...
                    className={`
                        relative group flex-1 flex items-center justify-center gap-1.5 h-12 rounded-t-lg transition-all duration-300 min-w-[80px]
