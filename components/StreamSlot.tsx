...
const StreamSlot: React.FC<StreamSlotProps> = ({ 
  ...
  return (
    ...
    >
      ...
      >
          {/* TOP BAR: Drag Handle (Left) + Actions (Right) */}
          <div className="w-full flex justify-between items-start">
              
              {/* DRAG HANDLE (Top Left) */}
              <div 
                className={`pointer-events-auto cursor-grab active:cursor-grabbing text-white/30 hover:text-white transition-colors duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                      ...
                      <AnimatePresence>
                        {hoveredAction === 'hide' && (
                            <motion.div
                                ...
                                className="absolute right-0 top-0 mt-2 px-2 py-1 bg-black/90 border border-white/10 rounded-md whitespace-nowrap z-[70] pointer-events-none"
                            >
