...
const StreamSlot: React.FC<StreamSlotProps> = ({ streamer, currentPlatform, onPlatformChange }) => {
  ...
  // Removed IntersectionObserver (Lazy Loading) to guarantee immediate loading of players.
  // This resolves issues where the observer might not trigger correctly in some layouts.
  ...
  const embedUrl = useMemo(() => {
    // Basic validation
    if (!channelId || channelId.includes('Inserir ID')) return '';
    ...
    if (typeof window !== 'undefined') {
        ...
        if (hostname) {
            ...
            // Handle subdomains/www robustly
    ...
    switch (currentPlatform) {
      case Platform.Twitch:
        // Changed muted=false to muted=true. 
        // Modern browsers BLOCK autoplaying video with sound. 
        // This is the #1 reason Twitch embeds fail to load (black screen).
        return `https://player.twitch.tv/?channel=${channelId}&${twitchParentParams}&muted=true&autoplay=true`;
        
      case Platform.YouTube:
        const origin = typeof window !== 'undefined' ? window.location.origin : '';
        // origin param helps with CORS/embed restrictions
  ...
  const hasValidChannel = Boolean(channelId && !channelId.includes('Inserir ID'));

  return (
    <div 
        className="relative w-full h-full bg-black overflow-hidden group min-h-[200px]"
    >
      
      {/* 1. VIDEO LAYER (Z-0) */}
      <div className="absolute inset-0 z-0 bg-black" style={{ pointerEvents: 'auto' }}>
         {hasValidChannel ? (
            <iframe
              ...
              allowFullScreen
              // Extensive permissions list to ensure no browser blocks
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen"
              // Removed sandbox attribute completely to avoid any restrictions on scripts/popups required by players
            />
         ) : (
           <div className="flex flex-col items-center justify-center w-full h-full text-white/10 select-none">
                ...
                <span className="text-sm font-medium tracking-wider mt-2">Offline / Configurar ID</span>
           </div>
         )}
      </div>

      {/* 2. HUD ELEMENTS (Z-10) - Floating buttons */}
      {/* Badge */}
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2 pointer-events-none">
          <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-white/50 hover:text-white hover:bg-white/10 transition-colors cursor-pointer pointer-events-auto"
