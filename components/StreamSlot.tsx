...
const StreamSlot: React.FC<StreamSlotProps> = ({ streamer, currentPlatform, onPlatformChange }) => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true);
          // Once loaded, we can stop observing to prevent reload on scroll out
  ...
  return (
    ...
    >
      ...
      {/* 1. VIDEO LAYER (Z-0) */}
      {/* Absolute positioning with pointer-events-auto ensures clicks go to iframe */}
      <div className="absolute inset-0 z-0 bg-black">
         {isVisible && channelId ? (
            <iframe
              ...
              // Removed 'interactive-widget' as it sometimes conflicts with custom overlays
              // Ensure NO 'sandbox' attribute is present
            />
         ) : (
      ...
      {/* 2. HUD CONTAINER (Z-10) */}
      {/* pointer-events-none is CRITICAL here. It lets clicks pass through the empty space to the video. */}
      <div className="absolute inset-0 z-10 pointer-events-none p-4 flex flex-col justify-between">
          ...
          <div className="flex justify-between items-start">
              ...
              {/* Controls Group - Clickable (pointer-events-auto) */}
